...
commands:
- name: da
  description: 花易项目数据库开发工具 — 安全连接 MySQL，查询表结构、获取示例数据
  script: scripts/da.py
  args:
  - --config
  - --db
  - --database
...
```
${COPILOT_PLUGIN_ROOT}/skills/database-access/scripts/da.py
${COPILOT_PLUGIN_ROOT}/skills/database-access/scripts/requirements.txt
```
...
```bash
pip install -r "${COPILOT_PLUGIN_ROOT}/skills/database-access/scripts/requirements.txt"
```
...
```bash
DA="${COPILOT_PLUGIN_ROOT}/skills/database-access/scripts/da.py"
