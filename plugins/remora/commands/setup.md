...
- `ready: true` —— 告诉用户 remora 已就绪，可以用 `/remora:task`。
- 配置缺失（`config error`）—— 提示用户设置 `REMORA_BASE_URL` / `REMORA_MODEL` 和 API key（`REMORA_API_KEY` 或 `~/.pi/remora.config.yaml` / `.remora/config.json` 的 `apiKey` spec：`keychain:SERVICE` / `env:VAR` / 裸 `VAR`；旧字段 `apiKeyEnv` 仍兼容），或创建 `~/.pi/remora.config.yaml`。
