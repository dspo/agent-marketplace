...
import { formatCommandFailure, runCommand, runCommandChecked, type RunCommandOptions } from "./process.ts";
...
export function ensureGitRepository(cwd: string): string {
  ...
  const errorCode = result.error && "code" in result.error ? (result.error as NodeJS.ErrnoException).code : null;
  if (errorCode === "ENOENT") {
...
export function resolveReviewTarget(cwd: string, options: { scope?: string; base?: string | null } = {}): ReviewTarget {
...
function collectWorkingTreeContext(cwd: string, state: WorkingTreeState, options: { includeDiff?: boolean } = {}): ContextDetails {
...
function collectBranchContext(cwd: string, baseRef: string, options: { includeDiff?: boolean; comparison?: BranchComparison } = {}): ContextDetails {
  const includeDiff = options.includeDiff !== false;
...
function buildCollectionGuidance(options: { includeDiff?: boolean } = {}): string {
...
export type ReviewContext = ContextDetails & {
  cwd: string;
  repoRoot: string;
  branch: string;
  target: ReviewTarget;
...
export function collectReviewContext(
  cwd: string,
  target: ReviewTarget,
  options: { includeDiff?: boolean; maxInlineFiles?: number; maxInlineDiffBytes?: number } = {}
): ReviewContext {
