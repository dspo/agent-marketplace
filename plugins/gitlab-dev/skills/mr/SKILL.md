---
name: mr
description: 实现需求并交付 MR —— 新建或继续 branch，实现变更，推送并创建/更新 MR
---

# 实现需求并交付 MR

当需要实现一个需求（来自 Issue、用户指令或已有讨论）并通过 GitLab MR 交付时，遵循本流程。
...
**如果已有相关 branch/worktree/MR**：
- 定位已有资产，在原有上下文中继续推进
- 最终更新同一个 MR，而不是另起一套交付物
...
> **强制要求**：所有代码变更必须在专用 worktree 中完成。严禁在主目录（root worktree）进行任何代码变更或实现工作，否则 MR 将无法正确关联到 branch，导致交付失败。
