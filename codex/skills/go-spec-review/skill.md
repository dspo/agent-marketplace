...
0) (Optional) Run helper scripts (fast context + red flags)
   - Scripts live under `${CODEX_PLUGIN_ROOT}/skills/go-spec-review/scripts/`.
   - `go run ${CODEX_PLUGIN_ROOT}/skills/go-spec-review/scripts/collect_context.go --repo <repo> --base <git-ref>`
   - `go run ${CODEX_PLUGIN_ROOT}/skills/go-spec-review/scripts/risk_sweep.go --repo <repo> --base <git-ref>`
   - `go run ${CODEX_PLUGIN_ROOT}/skills/go-spec-review/scripts/light_checks.go --repo <repo>`
   - Notes:
...
3) Run a "spec risk areas" sweep
   - Read `${CODEX_PLUGIN_ROOT}/skills/go-spec-review/references/spec-review-checklist.md` and scan for language-edge cases.
